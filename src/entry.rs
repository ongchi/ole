...
pub struct Entry {
    ...
    /// UID of the entry.
    identifier: Vec<u8>, // 16 bytes

    /// Flags of the entry.
    flags: Vec<u8>, // 4 bytes
...
impl Entry {
    fn from_slice(sector: &[u8], dir_id: u32) -> Result<Entry, super::error::Error> {
        let entry = Entry {
            ...
            left_child_node: u32::from_le_bytes(sector[68..72].try_into().unwrap()),
            right_child_node: u32::from_le_bytes(sector[72..76].try_into().unwrap()),
            root_node: u32::from_le_bytes(sector[76..80].try_into().unwrap()),
            identifier: sector[80..96].to_vec(),
            flags: sector[96..100].to_vec(),
            creation_time: u64::from_le_bytes(sector[100..108].try_into().unwrap()),
            last_modification_time: u64::from_le_bytes(sector[108..116].try_into().unwrap()),
            sec_id_chain: vec![u32::from_le_bytes(sector[116..120].try_into().unwrap())],
            size: u32::from_le_bytes(sector[120..124].try_into().unwrap()) as usize,
